...
if  [ ! -e '/tmp/speedtest.py' ]; then
    wget --no-check-certificate -P /tmp https://raw.github.com/afeigorgor/speedtest-cli/master/speedtest.py > /dev/null 2>&1
fi
...
if [[ ${telecom} =~ ^[1-3]$ ]]; then
    ...
    if [[ ${is_down} ]]; then
	    ...
	    echo -e "\n${RED}ERROR:${PLAIN} 当前节点不可用，请选择其他节点，或换个时间段再测试。"
	fi
fi
